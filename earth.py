...
ACTION_TABLE = {
	...
	'retrieve' :  earthpy.retrieve,    # retrieve tiles
	'clean' :  earthpy.clean,          # clean all cache the datasets
}
...
def cli():
	...
	parser.add_argument('action' , help='Available operations: ' + ' '.join(AVAILABLE_ACTIONS),
	...
	parser.add_argument('--dataset'  , help='Dataset to query raster tiles from.',
								   metavar=('dataset_id') , nargs=1  )

	parser.add_argument('--format'  , help='Raster tile format.',
								   metavar=('Format') , nargs=1 )

	parser.add_argument('--outdir' , help='Raster tile output directory',
								   metavar=('path') , nargs=1, default=['out'] )
	parser.add_argument('--prefix' , help='Prefix for raster file',
	...
	required_args = get_required_func_args(action)
	optional_args = get_optional_func_args(action)
	...
	if len(ignored_args) > 0:
		print('Unncessary arguments. Ignoring : ' + ' '.join(ignored_args))
